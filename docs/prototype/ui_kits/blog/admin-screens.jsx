  ...
  return (
    <div className="px-10 py-10 max-w-5xl">
      <header className="mb-8">
        <p className="text-[11px] uppercase tracking-wider font-semibold text-[var(--muted-foreground)] mb-1">Пользователи</p>
        <h2 className="font-[var(--font-display)] font-extrabold text-3xl md:text-4xl tracking-tight mb-2">Все аккаунты</h2>
        <p className="text-[14px] text-[var(--muted-foreground)] max-w-xl">
          Роли, статусы, последняя активность. Откройте карточку пользователя, чтобы изменить роль или заблокировать.
        </p>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        <input
          ...
          className="flex-1 min-w-[220px] bg-[var(--background)] border border-[var(--border)] rounded-md px-3 py-2 text-[13px] placeholder:text-[var(--muted-foreground)] focus:outline-none focus:border-[var(--accent)] transition-colors"
        />
      ...
      <ul className="divide-y divide-[var(--border)] border-y border-[var(--border)]">
        <li className="grid grid-cols-[1fr_120px_140px_120px] gap-4 py-2 px-1 text-[10.5px] uppercase tracking-wider font-semibold text-[var(--muted-foreground)]">
          <span>Пользователь</span>
          <span>Роль</span>
          <span>Статус</span>
          <span className="text-right">Последний визит</span>
        </li>
        {list.length === 0 && (
          <li className="py-6 text-center text-[13px] text-[var(--muted-foreground)]">Никого не нашлось.</li>
        )}
        {list.map(u => (
          <li key={u.handle}>
            <button
              onClick={() => onOpen(u.handle)}
              className="w-full grid grid-cols-[1fr_120px_140px_120px] gap-4 py-3 px-1 -mx-1 text-left rounded hover:bg-[var(--muted)]/40 transition-colors items-center"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <Avatar handle={u.handle} name={u.name} size={28} />
                <div className="min-w-0">
                  <p className="text-[13.5px] font-medium truncate">{u.name}</p>
                  <p className="text-[12px] text-[var(--muted-foreground)] truncate">@{u.handle}</p>
                </div>
              </div>
              <RolePill role={u.role} />
              <StatusDot status={u.status} />
              <span className="text-[12px] text-[var(--muted-foreground)] tabular-nums text-right">{u.lastSeenAt ? relTime(u.lastSeenAt) : "—"}</span>
            </button>
          </li>
        ))}
      </ul>
...
function AdminReports({ reports, users, onOpen }) {
  ...
  return (
    <div className="px-10 py-10 max-w-5xl">
      <header className="mb-8">
        <p className="text-[11px] uppercase tracking-wider font-semibold text-[var(--muted-foreground)] mb-1">Жалобы</p>
        <h2 className="font-[var(--font-display)] font-extrabold text-3xl md:text-4xl tracking-tight mb-2">Очередь модерации</h2>
        <p className="text-[14px] text-[var(--muted-foreground)] max-w-xl">
          Жалобы от читателей на комментарии и блоги. Откройте карточку, чтобы увидеть контекст и принять решение.
        </p>
      </header>
...
// (AdminReview + ReviewQueueRow were retired — the chapter-aware moderation
//  queue AdminReviewV2 / ChapterQueueRow in Admin-v3.jsx overrides window.AdminReview.)
