...
function ChapterReader({ blog, chapter, prevChapter, nextChapter, onPick, session, onLoginRequired, scrollRef, onOpenProfile, isMulti, author, totalVisibleChapters, visibleOrder }) {
  ...
  return (
    <article className="min-w-0">
      ...
      {/* Key skills (reader-visible, scientific-keyword style) */}
      {Array.isArray(chapter.skills) && chapter.skills.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 mb-8 -mt-3">
          <span className="text-[10.5px] uppercase tracking-wider font-semibold text-[var(--muted-foreground)] mr-1">Навыки</span>
          {chapter.skills.map(s => (
            <span key={s} className="text-[11.5px] px-2 py-0.5 rounded-full border border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--muted-foreground)]">{s}</span>
          ))}
        </div>
      )}
