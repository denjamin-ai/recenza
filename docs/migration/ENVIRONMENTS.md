...
- **Реальный IP клиента:** CF передаёт его первым элементом `X-Forwarded-For` — `clientKey()`
  в `src/lib/rate-limit.ts` читает именно первый хоп, логин-лимит работает. В логах Caddy
  `remote_ip` теперь = edge-IP Cloudflare. Backlog: ключевать по `CF-Connecting-IP`
  (неспуфится) и allowlist CF-диапазонов в ufw.
