...
С 2026-07-11 перед VPS стоит **прокси Cloudflare** (DNS + оранжевое облачко, SSL Full strict) —
причины и runbook в **§6.6**.
...
### 6.6 Cloudflare перед прод-стендом (DPI-инцидент 2026-07-11)

С 2026-07-11 прод стоит **за прокси Cloudflare**: браузер → Cloudflare edge → Caddy на VPS.
Caddy/деплой/env не менялись — поменялись только DNS и путь трафика.

**Проблема.** Вечером 2026-07-10 сайт перестал открываться в браузерах из РФ (curl при этом
работал). Диагноз по tcpdump на VPS: DPI-фильтрация на российском участке пути (ТСПУ) **съедала
второй TCP-сегмент больших браузерных TLS ClientHello** к IP VPS (диапазоны Aeza — частая
мишень). У Chromium-браузеров hello ~1.8КБ (пост-квантовый ключ ML-KEM) → идёт двумя
сегментами → первый доходит, хвост никогда → TLS-рукопожатие виснет вечно. Маленькие hello
(curl, старые клиенты) проходили; QUIC (HTTP/3, UDP 443) проходил; «мусорный» split-пакет
проходил — фильтр целился именно в настоящие браузерные hello. Проверено: через SOCKS мимо
российского участка сайт открывался → сервер/хостер чисты.

**Решение (по нарастающей).**
1. DNS-хостинг перенесён с Aeza на **Cloudflare** (NS `autumn`/`tate.ns.cloudflare.com`;
   регистратор прежний). Добавлена запись **HTTPS (тип 65)** `1 . alpn="h3,h2"` — Chrome/Firefox
   узнают о HTTP/3 из DNS и идут по QUIC, минуя убитый TCP.
2. Этого оказалось мало: **Яндекс Браузер (≈основная аудитория) не использует h3-подсказку из
   HTTPS-записи** (проверено эмпирически: ноль QUIC-пакетов на сервере при чистом профиле).
3. Поэтому включён **прокси Cloudflare (оранжевое облачко)** на записях `@` и `www` + режим
   **SSL/TLS = Full (strict)**. Браузеры ходят на IP Cloudflare (DPI их не фильтрует), CF ходит
   на origin с валидным LE-сертификатом Caddy. Бонус: CF-managed HTTPS-запись включает **ECH**
   (SNI шифруется — устойчивость к будущим SNI-блокировкам).

**Порядок включения прокси (важен!):** сначала SSL/TLS → Full (strict), потом облачко —
в дефолтном Flexible будет петля редиректов (Caddy принудительно редиректит на https).

**Следствия и runbook.**
- **Откат:** в CF DNS переключить записи на «DNS only» (серое облачко), TTL 300с. Сайт снова
  напрямую; для браузеров за DPI снова сломается.
- **Продление сертификата origin** (LE, истекает 2026-10-07): HTTP-01 через CF-прокси обычно
  проходит (порт 80 проксируется), но **в начале сентября проверить**
  `journalctl -u caddy | grep -i acme`; если продление падает — перевести Caddy на DNS-01
  (CF API token) или поставить Cloudflare Origin Certificate.
- **Реальный IP клиента:** ⚠️ **Прежняя формулировка этого пункта была ОШИБОЧНОЙ и закрепляла
  дырявую модель угроз** (исправлено аудитом ИБ 2026-07-26). Считалось, что CF «передаёт реальный IP
  первым элементом `X-Forwarded-For`», поэтому `clientKey()` читал первый хоп. На деле **первый хоп
  задаёт КЛИЕНТ**: если браузер прислал свой `X-Forwarded-For`, Cloudflare *дописывает* к нему адрес
  клиента, а Caddy дописывает адрес CF — подделанное значение так и остаётся слева. Ротация
  заголовка давала каждой попытке логина свежее ведро, и лимит 5/15мин не срабатывал никогда.
  Теперь `clientKey()` читает `CF-Connecting-IP` (CF перезаписывает его на edge — не подделывается),
  иначе **последний** хоп XFF, иначе `"local"`. Плюс добавлено пер-аккаунтное ведро (15/15мин),
  которое от IP не зависит вовсе. В логах Caddy `remote_ip` = edge-IP Cloudflare.
  Backlog (не сделано): allowlist CF-диапазонов в ufw — он закрыл бы прямой заход на origin в обход
  CF, где `CF-Connecting-IP` подделывается. ⚠️ Правила AmneziaWG (51820/udp, 51821/tcp) не трогать.
- **Кэш:** HTML не кэшируется (`Cf-Cache-Status: DYNAMIC`), статика `_next/static` кэшируется —
  безопасно (имена контент-хэшированы). HTML-мутирующие фичи CF (Rocket Loader и т.п.) НЕ включать.
- **CSRF/приложение:** CF сохраняет `Host` → same-origin проверка работает (проверено: POST без
  Origin → 403, с Origin → 401 на плохих кредах). Лимит тела CF (100МБ free) >> наших 8МБ.
- Клиентский обходной костыль на время инцидента (если прокси выключен, а DPI активен):
  `browser.exe --origin-to-force-quic-on=recenza.ru:443 https://recenza.ru` — один успешный
  h3-заход кэширует Alt-Svc на 30 дней.
