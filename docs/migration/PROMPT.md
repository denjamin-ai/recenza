...
| `README.md` (корень), `REVIEW-PROMPT.md` | Архив: bootstrap-гайд миграции; одноразовый аудит окружения | Только история |
...
1. Диагностика (read-only): curl -I https://recenza.ru ; gh run list --workflow=deploy --limit 3 ;
   при SSH-доступе — systemctl status recenza caddy, journalctl -u recenza -n 100, место на диске.
...
Прочитай @docs/migration/ENVIRONMENTS.md §6. Проверь по ситуации: HTTP-статусы и заголовки (curl),
состояние workflow (gh run list), сертификат/DNS, при SSH — сервис/таймеры (systemctl list-timers),
