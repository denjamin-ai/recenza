...
⚠️ **Фаза 14 сняла три механики целиком** — кейсов на них быть НЕ должно, а негативы на их роуты
ожидают **404**: подбор/пикер ревьюеров и `POST …/submit`, приглашения (`/api/reviewer/invitations/*`),
рейтинг ревьюеров (`POST /api/author/ratings`), «ведущий» (`…/primary-change`, `…/admin/review/…/primary`).
Отзыв `is_reviewer` теперь ОСВОБОЖДАЕТ: заявки возвращаются в очередь, назначения снимаются, `review_load` = 0.
...
| Оставлять ЗАЯВКУ на ревью (Ф14) | ❌ | ❌ | ✅ (в любом статусе главы, вкл. published) | ❌ | ❌ |
| Брать заявку из очереди (claim, Ф14) | ❌ | ❌ | ❌ (свой блог — 403) | ✅ пока `review_load < capacity` | ❌ |
| Выпускать инвайт-ссылку эксперта (Ф14) | ❌ | ❌ | ✅ (≤5 активных) | ❌ | ❌ |
| Анкета по инвайт-ссылке (Ф14) | ✅ публично | ✅ | ✅ | ✅ | — |
...
- **REV-QUEUE — заявка и очередь (Ф14, флагманский).** Автор оставляет ЗАЯВКУ на главу → она
  появляется в очереди кабинета ревьюера, отсортированная по совпадению с его компетенциями →
  ревьюер берёт её сам («Взять») → глава уезжает в «Мои ревью», ось ревью `in-review`.
  **Проверка:** автор нигде не выбирал исполнителя; claim перепроверяет capacity/свой-блог/актуальность
  ревизии на сервере; заявку можно оставить и на ОПУБЛИКОВАННУЮ главу, и тогда кредит с бейджем
  выдаются без повторной публикации.
- **REV-BADGE — бейдж как результат (Ф14).** approve → публикация → у главы «Проверено на Recenza»,
  на карточке блога чип; правка и публикация новой версии → бейдж главы становится «Проверена версия N
  от ‹дата› · текущая версия изменилась» со ссылкой `?v=N`, а бейдж БЛОГА сохраняется (историчен).
  Ревьюер, приведённый автором (`introduced_by`), даёт уровень «Проверено приглашённым экспертом».
- **REV-SLA — автоэскалация (Ф14).** Заявку никто не взял в срок → канал `editorial` + уведомление
  админу; взявший молчит → заявка возвращается в очередь, `review_load −1`, уведомления. Повторный
  тик крона идемпотентен.
...
- **PUB-ARTICLE — появление в каталоге**: опубликованная глава видна в КАТАЛОГЕ (`/?view=all`),
  у подписчиков — в их ленте, уведомление о новой главе. ⚠️ Ф15: на ГЛАВНУЮ публикация сама по себе
  больше не выводит — там витрина проверенных (см. FEATURED ниже).
...
### 3.2а Витрина и модерация (Фаза 15)

- **FEATURED — витрина главной.** На главную попадает только `verified_tier='independent'`;
  «проверено приглашённым экспертом» — нет. Пока проверенных меньше порога (3), витрину ведёт
  «Выбор редакции» (`blogs.featured_at`); нет ни того, ни другого → пустое состояние со ссылкой
  на каталог (отката на «показать всё» НЕТ). Каталог `/?view=all` отдаёт всё опубликованное.
- **NAV — навигация читателя.** Переходы «предыдущая/следующая глава» под текстом; `/blog/[slug]` —
  оглавление, а не редирект; `?mode=whole` сохранён; ссылки пейджера канонические даже из `?v=N`.
- **REPORT — жалобы.** Читатель жалуется на комментарий/блог, автор — приватно на ревьюера;
  админ разбирает (`resolve`/`delete_comment`/`hide_blog`). Негативы важнее happy-path: единый 404
  для «нет цели» и «нельзя», дедуп открытых жалоб, rate-limit.
...
- **COM-GATING — гейтинг.** ⚠️ Ф13 сняла ролевые запреты: комментирует любой вошедший аккаунт.
  403 дают только `commenting_blocked` и конфликт интересов (глава, которую зритель ревьюит/ревьюил).
...
| Витрина по бейджу (Ф15) | на главной только `verified_tier='independent'`; `invited` — по прямой ссылке и в профиле, но не на витрине |
| Пустая витрина (Ф15) | нет проверенных и нет закреплённых → пустое состояние + ссылка на каталог, а НЕ «показать всё» |
| SEO-расхождение (Ф15) | `feed.xml` — только проверенное, `sitemap.xml` — всё опубликованное |
| Жалоба как оракул (Ф15) | `POST /api/reports` на скрытую/удалённую/чужую цель → 404, тот же ответ, что и «нельзя» |
| Read-only админ-ревью (Ф15) | `/admin/review/[id]` открывается админом, но `POST /api/review/**` под админ-сессией → 401/403 |
| Claim нельзя обойти (Ф14) | claim при `review_load ≥ capacity` → 409; claim своего блога → 403; claim уже взятой → 409 — всё на сервере, не в UI |
| Бейдж нельзя подделать (Ф14) | ни один роут не принимает `verifiedTier`/`verifiedAt` из тела; уровень пишет только `closeReviewSession()` |
| Кредит не двоится (Ф14) | публикация + одобрение одной ревизии дают ОДИН кредит и ОДИН `reviewLoad −1` (токен `review_closed_at`) |
| Ревью после публикации (Ф14) | заявка на published-главу → claim → approve → кредит и бейдж появляются БЕЗ повторной публикации |
| Cron SLA защищён (Ф14) | `/api/cron/review-sla` без Bearer и с неверным → 401; отсутствие `CRON_SECRET` = 401, а не пропуск |
| Инвайт-токен не оракул (Ф14) | несуществующий, истёкший и использованный токены дают ОДИН ответ 409 «Ссылка недействительна» |
| Архивная версия только published (Ф14) | `?v=N` на неопубликованную ревизию → 404; страница `noindex` + canonical на текущую |
