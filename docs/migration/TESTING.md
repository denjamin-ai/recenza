...
| Голос/закладка/подписка | →login | ✅ | ❌ 403 | ❌ 403 | — |
...
- **REV-SESSION-CHAT — чат сессии.** Несколько ревьюеров + автор обсуждают в чате сессии (вне тредов).
  **Проверка:** сообщения видны всем участникам сессии; presence/typing; чат не смешивается с тредами.
  ⚠ UX-эталона чата в прототипе нет — UI проектируется в PLAN Фаза 6 (см. REVIEW-REPORT P2-10);
  сначала спроектировать и проверить через MCP, затем закрепить спеком.
...
|-----------|---------------|
| Редирект без сессии | GET `/admin`,`/author`,`/reviewer` без cookie → 307 (проверять **прямым GET**, без JS и без follow-redirects; Server Action даёт 303, `permanentRedirect()` — 308) |
