...
| 13 | Единый аккаунт, профиль и свободная публикация | Платформа | `in progress` |
| 14 | Ревью 2.0: заявки, каналы, бейджи | Продукт | `todo` |
| 15 | UX читателя/автора и кабинет администратора | Продукт | `todo` |
...
**DoD.**
- [ ] Любой аккаунт читает, комментирует, голосует, подписывается и может завести блог; автор видит чужие блоги.
- [ ] Автор публикует главу без единого ревьюера; опубликованную главу можно править (ревизия-черновик поверх).
- [ ] Профиль един, есть у любого аккаунта, редактируется владельцем (имя/био/ссылки/аватар/компетенции).
- [ ] `/workspace` собирает кабинеты возможностей; иконки-замка нет ни в одном из трёх мест.
- [ ] Имя ревьюера и автора комментария ведут на профиль из всех поверхностей.
- [ ] З-05 закрыт: вердикт не переживает повторную отправку.
- [ ] Доки и тест-документация синхронны коду; полный e2e зелёный.

**Журнал фазы.** (заполняется по ходу и при закрытии)
- Статус-история: `todo` → `in progress` (2026-07-25, сессия Фазы 13) → …
...
- Риски для следующих фаз:
  - **Ф13 PR-B:** профиль пока гейтится возможностями (аккаунт без них → 404) и аккаунт с ОБЕИМИ
    возможностями показывается только как авторский, без секции «что отрецензировал» — это ровно то,
    что должна закрыть подфаза 13.5 (профиль у любого аккаунта + `noindex` у пустого + таб «Ревью»).
  - **Ф13 PR-B:** `homeForCapabilities` временно не возвращает `/workspace` — вернуть при 13.6.
...
**DoD.**
- [ ] Автор нигде не выбирает ревьюеров; ревьюер берёт заявку сам из очереди по своим компетенциям.
- [ ] Заявку можно оставить на опубликованную главу; ревью не блокирует публикацию ни в одном сценарии.
- [ ] Рейтинг ревьюеров и «ведущий» удалены из кода и UI полностью (таблицы остались, чтения нет).
- [ ] Бейдж двух уровней виден читателю, привязан к номеру ревизии и не переживает правку.
- [ ] SLA возвращает зависшую заявку в очередь без участия человека.
- [ ] Кабинет ревьюера показывает блоги, где он участвует: активные и завершённые.
- [ ] Прототип §11 помечен superseded; доки и тесты синхронны.
...
- [ ] Применены скиллы `next-best-practices` + `security-checklist`
- [ ] Сабагент `code-reviewer`: нет P0/P1
- [ ] Сабагент `security-reviewer`: `POST /api/reports` — гейт/валидация/rate-limit/CSRF; выдача и
      отзыв возможностей только админом; admin-view ревью строго read-only
- [ ] Сабагент `design-watcher`: навигация по главам, футер, админ-плитки — токены, dark, 375px, aria
- [ ] Сабагент `seo-optimizer`: `sitemap.xml` отдаёт всё опубликованное, `feed.xml` — только проверенное,
      canonical/OG страницы блога
- [ ] Сабагент `playwright-tester`: полный регресс зелёный, 0 skip; GO
- [ ] Обновлены «Статус» и «Журнал фазы»

**DoD.**
- [ ] Главная — подборка проверенных блогов + «Выбор редакции»; непроверенный блог доступен по ссылке
      и виден в профиле автора с пометкой.
- [ ] Внизу главы работают переходы к предыдущей и следующей главе.
- [ ] Жалобу может создать читатель, и админ её разбирает — раздел перестаёт быть декорацией.
- [ ] Админ выдаёт и отзывает возможность ревьюера без бана; видит свои события; не имеет мёртвых ссылок.
- [ ] В админке нет ни одного элемента, управляющего удалённой сущностью.
- [ ] Все 62 замечания реестра закрыты либо осознанно перенесены в backlog с обоснованием.
