...
### 2026-07-09 · PR #TBD `feature-ui-feedback-3` (класс L, в работе)
- **Цель**: пакет из 13 замечаний владельца + сверка с прототипами (`docs/prototype/ui_kits/blog/src/**`;
  исключение — страница авторизации, она эталон): hover карточек (П1) · rename+DELETE черновиковых
  блогов (П2) · лимиты символов баннеров (П3) · CTA баннера по прототипу, всегда справа (П4) ·
  публичный профиль по `ProfileScreen` (П5) · AlphaBadge из админки долой (П6) · смена пароля
  админом (П7) · сгруппированное меню блоков редактора + click-outside (П8) · портирование
  «Руководства» (GuideModal) (П9) · автосейв структурных правок + сохранение на «Просмотр» (П10) ·
  единый BackLink (П11) · пикер ревьюеров по макету, дефолт «Все» (П12) · приветственный блог
  «О Recenza» на проде идемпотентным скриптом (П13).
- **Класс L**: новые API-поверхности (`DELETE /api/author/blogs/[blogId]`, `password` в
  `PATCH /api/admin/users/[handle]`), новый UI-раздел, контент-скрипт для прода. Миграций схемы нет.
- **Решения владельца**: DELETE — только полностью черновиковые блоги (все ревизии `draft`,
  `publishedAt IS NULL`), иначе 409; пикер ревьюеров — дефолтная вкладка «Все» (сортировка по «Топ»).
- **План тестов**: новый `flows/blog-manage.spec.ts` (rename/DELETE: happy+гейтинг+негатив);
  admin.spec — блоки «пароль» и «лимиты баннеров»; guide modal (guest+author); autosave + «Просмотр»
  в author.spec; обновление POM `editor.page.ts` и TC-AUTHOR-05; полный прогон зелёный.
- _Запись дополняется по факту закрытия итерации._
