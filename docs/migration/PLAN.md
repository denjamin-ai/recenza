...
### 2026-07-10 · `feature-ui-feedback-4` (класс L, в работе)
- **Цель**: пакет из 8 замечаний владельца, сверка с прототипами (эталон, кроме логина):
  П1 кабинет «Мои блоги» по `author-portal.jsx` (2 колонки карточек + aside 300px, футер карточки
  «＋ Глава»+pin) · П2 главная без табов «Лента/Каталог/Подписки» и без поиска, карточки БЛОГОВ
  (ролевой сплит: reader «Ваша лента» с «Подписки»/«Свежее», прочие «Все блоги» — решение владельца) ·
  П3 «Лента» в шапке справа · П4 donate-модалка по `donation-ui.jsx` (золотая шапка, карточки-ссылки,
  QR-сегменты) · П5 админ-кнопки: общий модуль + тона по прототипу (danger soft/strong, amber
  force-approve, ActionBtn-карточки жалоб) · П6 CTA карусели белым ПОД текстом слева (⚠️ реверс
  решения П4 из ui-feedback-3 «всегда справа» — новое указание владельца) · П7 recruit-слайд →
  тексты прототипа «Ищем ревьюеров / Стать ревьюером» (замена, не дубль — решение владельца;
  прод — идемпотентный upsert в `seed-recenza.mjs`) · П8 режим «Весь блог»: ОДИН merged-блок
  комментариев в конце + агрегированная карточка «Блог ревьюили» (реакции остаются per-chapter).
- **План тестов**: чинятся `feedTab`-локаторы (guest/author), TC-AUTHOR-01 (h1 «Мои блоги»),
  TC-ADMIN-25 (restore recruit-баннера из константы); новые кейсы — главная-каталог (гость/reader),
  merged-комментарии whole-mode (в `flows/review-whole-blog.spec.ts`), слайд «Ищем ревьюеров» → /board.
  Полный e2e зелёный, 0 skip.
