...
### 2026-07-26 · PR — `feature-ui-feedback-10` (класс M)

- **Задача владельца:** гость при заходе видит «Выбор редакции» (на проде витрина в режиме
  редакции) — «оставь просто Лента и фильтры (кроме фильтра подписок)». Уточнено: под единым
  h1 «Лента» гость видит ВИТРИННУЮ подборку одним списком (вариант «дефолт = фильтр
  Проверенные/Все» отклонён — он отменял бы З-19 и страховку R-2).
- **Сделано:** `Showcase` в `page.tsx` — h1 «Лента», один статичный лид, флэт-список
  `[...featured, ...blogs]` с общей пагинацией; режимный h1 и h2-секции «Выбор редакции»/
  «Проверенные блоги» удалены; `mode` больше не влияет на разметку (живёт внутри
  `getShowcase`/админки). Правила отбора Ф15 не тронуты.
- **Тесты:** `homeHeading` union сужен до «Лента»; TC-GUEST-01 (+негатив «у гостя нет чипа
  Подписки»), FEATURED-01 (негатив на заголовок «Выбор редакции»), FEATURED-04 переименован —
  проверяет состав после скрытия, а не смену h1; комменты seed.ts/TEST-PLAN обновлены.
- **Бэклог (P2 ревью):** `ShowcaseView.mode` после uif-10 не читается ни одним потребителем
  (page.tsx берёт только `featured`/`blogs`; `/admin/featured` считает `editorialLed` сам от
  `verifiedCount`) — убрать поле из публичного интерфейса при следующей уборке слоя showcase.

### 2026-07-26 · PR — `feature-ui-feedback-9` (класс M)

- **Задача владельца** (добивка единой ленты): (1) вычистить ВСЕ старые arrow-переходы —
  найден лид витрины гостя «…подборку ведёт редакция. Все блоги →»; (2) «Ваша лента» —
  переименовать или убрать: «теперь у нас везде одна общая лента, просто есть фильтры».
  Уточнено: h1 единый «Лента» везде (вариант «убрать совсем» отклонён — a11y); на витрине
  гостя вместо лид-ссылки — тот же чип-ряд.
- **Аудит переходов** (полный grep src): лид витрины «Все блоги →», кнопка пустой витрины
  «Все блоги →», «Найти блоги в каталоге →» пустых подписок — все три сняты, путь даёт чип
  «Все». Кнопка «Все блоги» на `/about` — CTA контентной страницы, оставлена (решение: не
  лентовый переход). Крошки/шапка «Лента» — не переходы, не тронуты.
- **Сделано:** h1 «Лента» на подписках И в каталоге (гостю тоже); SEO-title каталога не менялся
  («Все блоги | Recenza» описательнее); `FeedFilterNav.active` стал опциональным — на витрине
  гостя чипы БЕЗ активного (витрина ≠ состояние каталога: featured без бейджа есть, invited
  нет — `aria-current` врал бы); тексты пустых состояний переведены на «переключитесь на „Все"».
- **Тесты:** union `homeHeading` сужен до «Лента | Проверенные блоги | Выбор редакции»; там, где
  h1 перестал различать состояния, ассерты переведены на `aria-current` чипа + URL
  (TC-READER-21, TC-AUTHOR-23); FEATURED-03/05 обновлены (пустая витрина — чип вместо
  arrow-ссылки); в POM добавлен геттер `feedFilterNav`. TC-доки: TC-AUTHOR-23, TC-READER-07/23,
  TC-GUEST (устаревшие «главная-каталог» комменты).

### 2026-07-26 · PR — `hotfix-anchored-comment-flake` (класс S, тест-слой)

- **Инцидент процесса (PR #40):** smoke-гейт упал на TC-READER-09 (CI-флак: тройной клик до
  гидрации на медленном раннере не оставляет выделения → плавающая кнопка «Прокомментировать
  выделенный фрагмент» не появляется, оба ретрая одинаково), но merge всё равно прошёл —
  в автоматизации `gh pr checks --watch | tail` пайп проглотил код возврата, и `&&`-цепочка
  продолжилась. Прод не пострадал: дифф uif-8 зону комментариев не трогал, локальный полный
  прогон дважды 182/182, TC-READER-09 локально 3/3, деплой и прод-проверка зелёные.
- **Фиксы:** (1) `comments.page.ts` — пара «выделение → кнопка якоря» обёрнута в
  `expect().toPass` (штатный паттерн гидрационных флаков); (2) урок автоматизации — код
  возврата `gh pr checks` нельзя пропускать через пайп; проверять exit status отдельной
  командой до merge.

### 2026-07-26 · PR — `feature-ui-feedback-8` (класс M)

- **Задача владельца** (фидбек на ui-feedback-7): две hero-ссылки «Все блоги →»/«Проверенные →»
  избыточны — фильтр уже есть в каталоге. Решение владельца — **единая лента**: «для вида своих
  подписок также сделать фильтр переключаемый». Уточнено: дефолт — «Подписки»; секция открытий
  под подписками убирается; витрина гостя остаётся как есть (Ф15).
- **Сделано:**
  - Новый `feed-filter-nav.tsx` (заменил `catalog-filter-nav.tsx` из uif-7): чип-ряд
    «Подписки / Все / Проверенные», nav «Фильтр ленты». «Подписки» = `/` без параметров
    (личная лента, SEO не задет), «Все»/«Проверенные» — прежние URL каталога; чип «Подписки»
    в каталоге виден только вошедшему. Сортировка каталога при переключении сохраняется
    (`catalogQuery`).
  - `ReaderHome` перестроен: hero без ссылок, один список followed-блогов под чипами, h2
    «Подписки» и секция открытий с витрины удалены (`getShowcase` в ленте больше не зовётся);
    пустые подписки — прежнее пустое состояние со ссылкой в каталог.
  - Витрина гостя/админа, каталог, canonical, `getShowcase` — без изменений.
- **Тесты:** TC-READER-21 переписан под чипы (активный `aria-current`, переключение
  Подписки→Все→Проверенные→Подписки, негатив на h2 «Подписки»); FEATURED-03 — локаторы на
  «Фильтр ленты» + негатив «у гостя нет чипа „Подписки“». Проверены все `gotoFeed()`/`goto("/")`
  вошедших: ни один тест не полагался на снесённую секцию открытий. Негативы «Разделы ленты»
  остаются в силе (другое имя nav).
- **Доки:** CLAUDE.md (bullet uif-8, пометка о снятых hero-ссылках в uif-7), TC-READER-07/23,
  TEST-PLAN.
