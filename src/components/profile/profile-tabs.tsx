  ...
  return (
    <div>
      <div role="tablist" aria-label="Разделы профиля" className="flex gap-6 border-b border-[var(--border)]">
        <button type="button" role="tab" aria-selected={tab === "about"} onClick={() => setTab("about")} className={tabCls(tab === "about")}>
          Об авторе
        </button>
        <button type="button" role="tab" aria-selected={tab === "blogs"} onClick={() => setTab("blogs")} className={tabCls(tab === "blogs")}>
          Блоги
        </button>
      </div>
