  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        // ⚠️ Аудит ИБ 2026-07-26: блог, скрытый АДМИНОМ по жалобе, тоже не принимает комментарии.
        // Без этого фильтра модерация не останавливала создание контента на скрытом объекте, а
        // разница 200/404 выдавала его существование. Все остальные поверхности (feed.ts,
        // chapters.ts, reports.ts) фильтруют hidden — здесь фильтр был пропущен.
