...
- Версионирование: при обновлении главы — сначала снимок предыдущей ревизии в `chapter_revisions` (`prev_blocks`)
- Ролевой гейтинг (binding): admin API — `requireAdmin()`; author — ownership; reviewer — проверка назначения.
  Админ не создаёт блоги/главы; роль не редактируется обычным API; ревьюер не комментирует.
...
- Отсутствие снапшота ревизии при обновлении главы (версионирование)
...
❌ src/app/api/admin/users/[id]/route.ts:15 — PUT без requireAdmin()
...
💡 src/app/(reader)/blog/[slug]/page.tsx — нет loading.tsx
