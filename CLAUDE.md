...
## Текущее состояние репозитория (фазы 0–6 `done`, дальше 7–12 `todo`)
...
два стенда, auth/роли, читательский слой, авторский слой (кабинет/редактор/портфолио). npm-скрипты работают.
...
Следующая фаза — **7 (review-flow)**. Авторский кабинет, редактор (Variant B), `SubmitSheet` и
портфолио — уже **готовый код** (Фаза 6 `done`), не спецификация. Для остального (фазы 7–12) перед тем
как опереться на путь/таблицу/роут из «Архитектуры», **убедись, что фаза, вводящая его, уже `done`** —
часть описанного пока только спецификация.
