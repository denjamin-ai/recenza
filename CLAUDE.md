...
- **ui-feedback-5**: **голоса — БЛОГОВЫЕ** (`blog_votes`; миграция 0006 сконвертировала chapter_votes
  ...
  подписка) — только роль reader** (`requireUser("reader")` в vote/bookmarks/follows; `/bookmarks` —
  307 для не-reader; «Закладки» в меню — только reader); бар «Реакции» рендерится ОДИН раз:
  whole-режим — в шапке блога, режим главы — после контента (`canEngage` = гость|reader;
  engagement считается страницей, НЕ в `buildReaderSections`). **Аватарки**: kind=avatar в
  `/api/uploads` (любой пользователь) + `PATCH /api/profile/avatar` (self, строго `/uploads/avatars/`);
  UI — `avatar-changer.tsx` (menuitem в меню аватара для всех ролей + кнопка на своём `/u/…`);
  ...
  по прототипу (центр, метрики, «Список ведёт редакция»); в админке создание направления — наверху
  секции + ссылка «Открыть доску →».
