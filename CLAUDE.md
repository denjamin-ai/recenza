...
⚠️ **Бэклог пересмотрен целиком 2026-07-26** — итог по каждому пункту в `PLAN.md` §«Ревизия
бэклога» (что сделано, что устарело вместе с механикой, что отложено и по какому триггеру
возвращается). Прежде чем брать «долг» из журнала фазы — свериться с этой таблицей.
...
- Один спек/тест: `npx playwright test testing/e2e/<файл>.spec.ts` или `--grep "<название теста>"`
  (мутирующие спеки — serial с `reseed()` в `beforeAll`/`afterAll`, одиночный прогон файла безопасен)
...
- **Блоки — структурный JSON, MDX-конвейера НЕТ** (аудит ИБ 2026-07-26 снял давний дрейф: пакеты
  `next-mdx-remote`/`rehype-pretty-code` были объявлены, но не имели НИ ОДНОЙ ссылки в `src/` —
  удалены). Блоки рендерятся в React-узлы (`block-renderer.tsx`), текст экранируется автоматически —
  это строго безопаснее MDX. Подсветка кода — `shiki` напрямую (RSC), формулы — `katex` (RSC),
  диаграммы — `mermaid` (клиентский ленивый). Рендер блоков идентичен в ридере и ревью
...
- ⚠️ Регресс-ловушка: роут `article` обязан рендерить data-driven `BlogReaderScreen`, не легаси
  single-article вид (см. `docs/prototype/README.md` §3 «Screen map»). Открытие разных блогов → разный контент, обновление `title`/OG.
...
- ⚠️ **uif-10: витрина = ПРАВИЛА ОТБОРА, а не заголовки.** h1 главной гостя — всегда «Лента»
  (как у всех), подборка идёт одним списком (закреплённые первыми, затем independent);
  режимные h1/h2 «Проверенные блоги»/«Выбор редакции» с главной УДАЛЕНЫ. «Выбор редакции»
  остаётся концептом админки (`/admin/featured`). Всё ниже — про ОТБОР, он не менялся.
...
### Инварианты безопасности (binding; аудит ИБ 2026-07-26)
Сквозной аудит подтвердил главное свойство — **доступа к чужим данным нет**: IDOR не найдено,
ownership проверяется на сервере, возможности перечитываются из БД на каждый запрос, raw SQL и
`JSON.parse` вне `db/json.ts` отсутствуют, секреты в git не попадали. Ниже — то, что было исправлено
и переставать соблюдать нельзя.

- **IP для rate-limit — только доверенный хоп** (`clientKey`, `src/lib/rate-limit.ts`):
  `CF-Connecting-IP` → иначе **ПОСЛЕДНИЙ** хоп `X-Forwarded-For` → `"local"`. ⚠️ **Первый хоп XFF
  задаёт КЛИЕНТ** — Cloudflare и Caddy *дописывают* свой адрес к присланному. Пока ключом был первый
  хоп, ротация заголовка давала каждой попытке свежее ведро и лимит логина не срабатывал никогда.
- **Логин лимитируется ДВУМЯ вёдрами**: по IP (5/15мин) и **по аккаунту** (`acct:<handle>`,
  `acct:admin`, 15/15мин). Пер-аккаунтное ведро не зависит от IP — только оно закрывает
  распределённый перебор. ⚠️ Обратная сторона (принята владельцем): аккаунт, включая админский,
  можно запереть снаружи на 15 минут. **Аварийный выход — `sudo systemctl restart recenza`**
  (вёдра in-memory). Промах логина обязан платить `bcrypt.compare` против фиктивного хэша —
  иначе тайминг выдаёт существование аккаунта.
- **Гейт возможности проверяется в ОБЕИХ ветках `resolveReviewAccess`** — и у ревьюера
  (`isReviewer`), и у автора (`canAuthor`). Без второй проверки автор со снятой возможностью
  сохранял запись в `/api/review/**`, включая `threads/[id]/apply`, который правит блоки главы.
- **`blogs.hidden` — часть гейта СОЗДАНИЯ контента, а не только чтения.** Скрытый админом блог не
  принимает комментарии, голоса и закладки. Общий предикат видимости для реакций —
  `resolveEngageableBlog()` (`queries/engagement.ts`); свой блог не голосуется (накрутка витрины).
- **Единый 404 на «нет объекта» и «объект не виден»** — иначе роут становится оракулом
  существования скрытого. Гард доступа идёт ПЕРЕД любым 404/409 о состоянии объекта
  (треды ревью: `requireUser` → `resolveReviewAccess` → и только потом статус треда).
- **Ссылки в тексте блоков**: http(s) либо **одиночный** `/`. `//host` и `/\host` —
  протокол-относительные ВНЕШНИЕ адреса, рендерятся литералом (`inline.tsx`, та же политика в
  `markable-textarea.tsx` и `lib/url.ts`).
- **CSP — nonce-based, `src/middleware.ts`.** ⚠️ Middleware существует ТОЛЬКО ради заголовков и
  **не является гейтом аутентификации** — гейтинг остаётся пороутовым. `next-themes` обязан
  получать nonce (`ThemeProvider nonce={...}` из `headers().get("x-nonce")`), иначе тема не
  применяется до гидрации. ⚠️ **`<script type="application/ld+json">` nonce НЕ ставить**: это
  data-block (CSP его не блокирует), а браузер скрывает значение nonce от чтения из DOM →
  hydration mismatch на каждой странице.
- **Загрузки**: `Content-Length` обязателен (иначе chunked-тело проходит мимо раннего 413 прямо в
  `formData()`); allowlist'ы через `Object.hasOwn`, не `in`; сигнатуры проверяются целиком.
  ⚠️ `/uploads/*` в проде отдаёт **Caddy мимо Next** — security-заголовки для них живут в
  `deploy/Caddyfile`, а не в `next.config.ts`. Правишь одно — правь другое.
- **Троттлинг heartbeat — условным UPDATE, а не in-memory лимитом**: in-memory «пропустил бы»
  запись молча, и presence разъезжался бы с реальностью.
...
- Фикстуры (`fixtures.ts`): `asGuest/asReader/asAuthor/asReviewer/asAdmin` (свой browserContext поверх
  storageState), `loginAs(handle)` (`sergey_review`/`lena_review`/`max_review`/`troll`), `guestWithXff(xff)` (изоляция login-лимита),
...
- **ui-feedback-4 (сверка с прототипами)**: **главная без табов и поиска, карточки БЛОГОВ** —
  сплит в `app/(reader)/page.tsx` (⚠️ после Ф15 он «вошёл/не вошёл», НЕ ролевой): любой вошедший →
  «Ваша лента» (секции «Подписки» + «Проверенные блоги»/«Выбор редакции»; секции «Свежее» НЕТ),
  гость/админ → витрина Ф15; каталог `/?view=all` общий для всех;
...
- **ui-feedback-6**: **админ-страница «Доска ревьюеров»** (`/admin/board`, раздел «Платформа») —
  ...
  компоненты убраны, «Заявки ревьюеров» = только запросы+отклики). **Автор без «Ленты»** —
  ⚠️ РЕВЕРСНУТО (uif-6 П6): «Лента» в шапке снова у ВСЕХ, пункта «Все мои блоги» в меню нет,
  h1 каталога всегда «Все блоги» (З-07). **«Изменить профиль» на `/u/` убран** — редактирование
  живёт в табе «Об авторе» (AuthorProfile). **Футер с тэглайном удалён** (AppFrame без `<footer>`;
  Ф15 вернула служебный `site-footer.tsx`).
- **ui-feedback-7 (2026-07-26)**: **меню аватара** (`avatar-menu.tsx`) — порядок «Мой профиль ·
  Рабочее место · Закладки · Настройки · Выйти»; пункты «Кабинет автора»/«Кабинет ревьюера» и
  строка возможностей в шапке панели УДАЛЕНЫ — в кабинеты ведут карточки «Рабочего места»
  (пункт по-прежнему виден только при наличии возможностей; e2e фиксирует состав И порядок
  одним `toHaveText`). **Гид** (`guide-modal.tsx`): кабинетных CTA нет — `cta` рендерится
  только при `caps.length === 0` (гость → «Войти», читатель → «Доска»); «Понятно»/крестик/
  фон/Escape закрывают. **Фильтр каталога** `/?view=all&filter=verified`: «проверен» = ЛЮБОЙ
  бейдж (`isReviewVerified`, оба tier) — это НЕ витринное правило (`isShowcaseVerified` =
  только independent — пропуск на главную, З-19); nav называется «Фильтр каталога» (⚠️ НЕ
  «Разделы ленты» — на это имя стоят негативные e2e-ассерты); query каталога/витрины собирает
  ЕДИНСТВЕННЫЙ `catalogQuery()` (`lib/showcase.ts`) — canonical, чипы и пагинация обязаны
  строить URL одинаково, иначе canonical перестаёт совпадать сам с собой. (Hero-ссылки
  «Все блоги →»/«Проверенные →» из этой итерации сняты в ui-feedback-8 — их заменил чип-ряд.)
- **ui-feedback-8 (2026-07-26): ЕДИНАЯ лента вошедшего.** Главная `/` вошедшего — один список
  под чип-рядом «Подписки / Все / Проверенные» (`feed-filter-nav.tsx`, nav «Фильтр ленты» —
  ⚠️ НЕ «Разделы ленты», негативные e2e; `catalog-filter-nav.tsx` удалён). Дефолт —
  «Подписки» (`/` без параметров, followed-блоги БЕЗ витринного фильтра); «Все»/«Проверенные» —
  те же URL каталога (`/?view=all[&filter=verified]`), чип «Подписки» в каталоге виден только
  вошедшему. Секция открытий («Проверенные блоги»/«Выбор редакции» под подписками) и h2
  «Подписки» УДАЛЕНЫ — один список на состояние. Витрина гостя/админа — без изменений (Ф15).
- **ui-feedback-9 (2026-07-26): h1 «Лента» + чипы вместо arrow-ссылок ВЕЗДЕ.** Заголовок ленты
  ЕДИНЫЙ — «Лента» и на подписках (`/` вошедшего), и в каталоге (`?view=all`, в т.ч. гостю);
  состояние различают чипы (`aria-current`) и URL, НЕ h1 («Ваша лента» и «Все блоги» как h1
  больше не существуют; union `homeHeading` в POM — «Лента | Проверенные блоги | Выбор
  редакции»). SEO-title каталога остался «Все блоги | Recenza» (описательность > совпадения
  с h1). Все arrow-переходы лентовых поверхностей СНЯТЫ: лид витрины и кнопка пустой витрины
  («Все блоги →»), «Найти блоги в каталоге →» пустых подписок — путь везде даёт чип «Все».
  На витрине гостя чип-ряд «Все / Проверенные» рендерится БЕЗ активного чипа
  (`FeedFilterNav.active` опционален): витрина ≠ состояние каталога (featured без бейджа в ней
  есть, invited нет) — `aria-current` там врал бы. Кнопка «Все блоги» на `/about` — CTA
  контентной страницы, не лентовый переход, оставлена.
- **ui-feedback-10 (2026-07-26): у гостя тоже h1 «Лента», витрина без заголовков.** Режимный
  h1 («Проверенные блоги»/«Выбор редакции») и h2-секции с главной гостя удалены — витринная
  подборка Ф15 идёт ОДНИМ списком (`[...featured, ...blogs]`, дедуп в `getShowcase`), с общей
  пагинацией. Правила отбора binding-раздела Ф15 НЕ менялись (invited по-прежнему не на
  главной, страховка редакции жива — просто невидима как рубрика). `homeHeading` в POM сузился
  до единственного «Лента»; e2e режимного заголовка (FEATURED-04) переориентирован на состав.
...
- **Модалки — общий хук `src/lib/use-modal-a11y.ts`** (Ф15.1): Escape + автофокус + циклический
  Tab (focus-trap) + возврат фокуса на триггер. ⚠️ Локальных `useEffect`-обвязок больше не заводить:
  до него каждая модалка городила свою, а полноценного trap'а не было ни в одной (пункт переносился
  по бэклогу с Ф6). Диалогу нужны `role="dialog"`, `aria-modal`, `tabIndex={-1}` и ref из хука.
- **Табы — roving `tabindex`** (кабинет ревьюера, профиль): по Tab заходит только активный таб,
  между табами — ←/→/Home/End. Новые tablist'ы делать так же.
