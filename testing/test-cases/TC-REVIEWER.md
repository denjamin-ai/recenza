...
3. Открыть меню аватара (кнопка с aria-label «Меню пользователя») в шапке.
   **Expected:** В меню есть «Рабочее место» и «Выйти»; кабинетных пунктов нет ни у кого (ui-feedback-7) — в кабинет ревьюера ведёт карточка на `/workspace`.
4. Нажать «Выйти».
...
3. Открыть `/reviewer` и главную `/`; осмотреть шапку и меню аватара («Меню пользователя»).
   **Expected:** Нигде нет кнопок/ссылок «Создать блог», «Новый блог», «Написать главу»; в меню — «Рабочее место» и «Выйти», кабинетных пунктов нет (ui-feedback-7; авторской карточки нет и на `/workspace`).
