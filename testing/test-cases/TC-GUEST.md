...
## TC-GUEST-01: Лента и каталог доступны гостю, карусель промо-баннеров видна

**Priority:** P1
**Type:** Functional / UI

### Objective
Гость без сессии видит главную страницу: табы «Лента» / «Каталог» / «Подписки», опубликованный контент и карусель промо-баннеров (в т.ч. баннер «Стать ревьюером», переехавший из шапки).
...
1. Открыть `http://localhost:3001/`.
   **Expected:** Страница отдаётся без редиректа; в шапке — ссылка «Войти» (меню пользователя отсутствует); активен таб «Лента» со списком опубликованных глав — видна глава «Цикл событий» блога «Глубоко в асинхронность JavaScript».
2. Осмотреть карусель промо-баннеров над/в ленте.
   **Expected:** Карусель отображается; среди слайдов — баннер `pb_recruit` «Станьте ревьюером Recenza» с CTA «Подать заявку», партнёрский `pb_partner` и donate-баннер `pb_donate`.
3. Кликнуть CTA баннера `pb_recruit` («Подать заявку»).
...
4. Вернуться на `/`, дождаться/долистать до donate-баннера и кликнуть его CTA.
   **Expected:** Открывается модалка «Поддержать Recenza» (aria-label «Поддержать проект»): кнопка-ссылка DonationAlerts (`dm_link`) и QR СБП (`dm_qr`); суммы не запрашиваются.
5. Закрыть модалку, переключиться на таб «Каталог».
   **Expected:** Сетка карточек блогов; видна карточка «Глубоко в асинхронность JavaScript» (slug `async-deep-dive`) с тегами и сложностью.
6. Переключиться на таб «Подписки».
   **Expected:** Вместо списка — приглашение войти (ссылка «Войти» с `next=/?tab=subscriptions`); подписки гостю недоступны.
...
7. Открыть `/` → таб «Подписки».
   **Expected:** В подписках видны главы автора `author` (в т.ч. «Цикл событий»).
...
### Test Steps
1. Открыть `/`, осмотреть таб «Лента».
   **Expected:** Глав блога «Скрытый блог» нет в списке.
2. Переключиться на таб «Каталог».
   **Expected:** Карточки «Скрытый блог» (`hidden-blog`) нет.
