...
4. Открыть `/`.
   **Expected:** Секции «Подписки» нет (подписок ноль — пустое состояние «У вас пока нет подписок»); блог виден во второй секции («Проверенные блоги» / «Выбор редакции» — Ф15, секции «Свежее» больше нет).
...
### Objective
Читатель на главной видит «Вашу ленту»: секция «Подписки» показывает блоги авторов, на которых он
подписан, **независимо от бейджа** (подписка — явный выбор читателя), а секция открытий подчиняется
витринной политике. Каталог «Все блоги» остаётся доступен по ссылке.

### Test Steps
1. Войти читателем, открыть `/`.
   **Expected:** h1 «Ваша лента»; секция «Подписки» с блогом автора, на которого есть подписка —
   даже если у блога нет бейджа.
2. Осмотреть вторую секцию.
   **Expected:** «Проверенные блоги» (или «Выбор редакции», если проверенных меньше трёх) —
   без блогов авторов из подписок и без уровня «приглашённым экспертом».
3. Кликнуть «Все блоги →».
   **Expected:** `/?view=all`, h1 «Все блоги», видно ВСЁ опубликованное; работают чипы сортировки
   и постраничная навигация.
4. Вернуться на `/`, кликнуть «Проверенные →» в hero (ui-feedback-7).
   **Expected:** `/?view=all&filter=verified` — в каталоге только блоги с бейджем ревью (оба
   уровня, включая «приглашённым экспертом»); чипы «Все / Проверенные» (nav «Фильтр каталога»)
   переключают фильтр, сортировка при этом сохраняется.
