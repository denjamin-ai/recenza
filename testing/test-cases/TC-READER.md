...
### Test Steps
1. Открыть ридер `http://localhost:3001/blog/async-deep-dive/event-loop`; панель `aria-label="Реакции"` — НАВЕРХУ главы, после заголовка/навыков и ДО контента (ui-feedback-6 П1).
   **Expected:** Панель видна до первого блока контента; кнопка ▲ (`aria-label="Полезно"`) в активном состоянии (`aria-pressed="true"`, акцентная рамка); счётчик = 1.
2. Нажать ▲ («Полезно»).
...
### Test Steps
1. Открыть `/` — главная читателя (uif-9: h1 «Лента», единая лента, чип «Подписки» активен).
   **Expected:** В выдаче «Подписок» видна карточка блога `async-deep-dive` (автор `author`).
...
4. Открыть `/`.
   **Expected:** Пустое состояние «У вас пока нет подписок — переключитесь на „Все", чтобы найти блоги.» (uif-9: arrow-ссылок нет, путь в каталог — чип «Все»); блог доступен через чипы «Все»/«Проверенные».
5. Вернуться в ридер и нажать «Подписаться на автора» (пауза ≥1 сек от шага 3).
   **Expected:** Кнопка снова «Вы подписаны»; на главной в выдаче «Подписок» блог вернулся.
...
3. Открыть `/?view=all` (каталог; uif-9: h1 «Лента», активный чип «Все»).
...
## TC-READER-23: Витрина, каталог и подписки для вошедшего читателя (Фаза 15)

**Priority:** P1
**Type:** Functional / UI

### Objective
ui-feedback-8: главная вошедшего — ЕДИНАЯ лента с чип-рядом «Подписки / Все / Проверенные»
(nav «Фильтр ленты»). Дефолт — «Подписки»: блоги авторов из подписок **независимо от бейджа**
(подписка — явный выбор читателя). «Все»/«Проверенные» — те же URL каталога.

### Test Steps
1. Войти читателем, открыть `/`.
   **Expected:** h1 «Лента» (uif-9: заголовок един на всех состояниях); чип «Подписки» активен
   (`aria-current`); один список — блоги авторов из подписок, даже без бейджа. Отдельных секций
   и блока открытий нет.
2. Кликнуть чип «Все».
   **Expected:** `/?view=all`, h1 остаётся «Лента», активен чип «Все»; видно ВСЁ опубликованное;
   работают чипы сортировки и постраничная навигация; в чип-ряду у вошедшего есть «Подписки».
3. Кликнуть чип «Проверенные».
   **Expected:** `/?view=all&filter=verified` — только блоги с бейджем ревью (оба уровня, включая
   «приглашённым экспертом»); сортировка при переключении сохраняется.
4. Кликнуть чип «Подписки».
   **Expected:** возврат на `/` — чип «Подписки» снова активен (h1 не меняется). У гостя чипа
   «Подписки» нет (личной ленты нет); витрина гостя — та же (Ф15), но с uif-9 на ней есть
   чип-ряд «Все / Проверенные» БЕЗ активного (замена лид-ссылки «Все блоги →»).

---

## TC-READER-24: Жалоба на комментарий из ридера (Фаза 15)

**Priority:** P1
**Type:** Functional / Security

### Objective
Любой вошедший аккаунт может пожаловаться на чужой комментарий; на свой — кнопки нет.
Гостю кнопка не показывается вовсе (роут ответил бы 401).

### Test Steps
1. Читателем открыть главу с комментариями.
   **Expected:** У чужого комментария есть «Пожаловаться», у своего — нет.
2. Нажать «Пожаловаться».
   **Expected:** Модалка «Пожаловаться на комментарий»: радио-причины (спам / оскорбления /
   не по теме / другое) и необязательный «Комментарий модератору».
3. Выбрать причину и отправить.
   **Expected:** Модалка закрывается, на месте кнопки — «Жалоба отправлена». Повторная жалоба на ту
   же цель новой строки у админа не создаёт.
4. Открыть страницу гостем.
   **Expected:** Кнопки «Пожаловаться» нет.
