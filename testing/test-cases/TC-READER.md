...
### Test Steps
1. Открыть `/` и в навигации `aria-label="Разделы ленты"` переключиться на таб «Подписки» (`?tab=subscriptions`).
   **Expected:** В ленте подписок видны опубликованные главы блога `async-deep-dive` (автор `author`).
...
4. Открыть `/` → таб «Подписки».
   **Expected:** Лента подписок пуста (пустое состояние), глав `async-deep-dive` нет.
5. Вернуться в ридер и нажать «Подписаться на автора» (пауза ≥1 сек от шага 3).
   **Expected:** Кнопка снова «Вы подписаны»; в табе «Подписки» главы блога вернулись.
