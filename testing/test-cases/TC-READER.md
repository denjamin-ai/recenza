...
### Test Steps
1. Открыть `/` — главная читателя «Ваша лента» (ui-feedback-8: единая лента, чип «Подписки» активен).
...
4. Открыть `/`.
   **Expected:** Пустое состояние «У вас пока нет подписок» + ссылка в каталог (ui-feedback-8: единая лента, отдельных секций и блока открытий больше нет); блог доступен через чипы «Все»/«Проверенные».
...
1. Войти читателем, открыть `/`.
   **Expected:** h1 «Ваша лента»; чип «Подписки» активен (`aria-current`); один список — блоги
   авторов из подписок, даже без бейджа. Отдельных секций и блока открытий нет.
2. Кликнуть чип «Все».
   **Expected:** `/?view=all`, h1 «Все блоги», видно ВСЁ опубликованное; работают чипы сортировки
   и постраничная навигация; в чип-ряду у вошедшего есть «Подписки».
...
4. Кликнуть чип «Подписки».
   **Expected:** возврат на `/` — «Ваша лента». У гостя чипа «Подписки» нет (личной ленты нет),
