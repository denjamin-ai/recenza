...
export class ReaderPage {
  ...
  /**
   * ui-feedback-9: h1 ленты ЕДИНЫЙ — «Лента» и на подписках (`/` вошедшего), и в каталоге
   * (`?view=all`); состояние различают чипы «Фильтра ленты» (`aria-current`) и URL, не заголовок.
   * ⚠️ Ф15: гостю главная отдаёт ВИТРИНУ: «Проверенные блоги» либо «Выбор редакции»
   * (когда проверенных меньше порога).
   */
  homeHeading(name: "Лента" | "Проверенные блоги" | "Выбор редакции"): Locator {
    return this.page.getByRole("heading", { level: 1, name });
  }

  /** Чип-ряд единой ленты (ui-feedback-8/9); имя намеренно НЕ «Разделы ленты». */
  get feedFilterNav(): Locator {
    return this.page.getByRole("navigation", { name: "Фильтр ленты" });
  }
